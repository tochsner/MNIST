...
class HebbianNetwork(SimpleNeuronalNetwork): 
    def __init__(self, size, activation_function):
        SimpleNeuronalNetwork.__init__(self, size, activation_function, activation_function, activation_function)

    def get_derivatives(self, output_derivatives):
        return output_derivatives
        
    def train_network(self, input_values):
        # feed-forward pass
        self.get_output(input_values)

        # change weights
        return self.calculate_new_weights()
        
    def calculate_new_weights(self):
        for i in range(0, self.layer_count - 1):
            self.new_weights[i] += sp.stats.zscore(self.weights[i] * self.neurons[i][np.newaxis].T) * self.neurons[i + 1].T
